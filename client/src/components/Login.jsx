...
const Login = () => {
  ...
  return (
    <div onClick={()=> setShowLogin(false)} className='fixed top-0 right-0 bottom-0 left-0 z-100 flex items-center text-sm text-gray-600 bg-black/50'>
    ...
    <form onSubmit={onSubmitHandler} onClick={(e)=>e.stopPropagation()} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white">
            ...
            <div className="w-full ">
                <p>Email</p>
                <input onChange={(e) => setEmail(e.target.value)} value={email} placeholder="Enter your email" className="border border-gray-200 rounded w-full p-2 mt-1 outline-blue-600" type="email" required />
            </div>
            <div className="w-full ">
                <p>Password</p>
                <input onChange={(e) => setPassword(e.target.value)} value={password} placeholder="Password" className="border border-gray-200 rounded w-full p-2 mt-1 outline-blue-600" type="password" required />
            </div>
            ...
            className='mb-4 text-blue-600 cursor-pointer'>Forgot password?</p>

            <button className="bg-blue-600 hover:bg-blue-600 transition-all text-white w-full py-2 rounded-md cursor-pointer">
