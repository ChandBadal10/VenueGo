...
const VenueDetails = () => {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          if (allRes.data.success) {
            ...
            const matchingVenues = allRes.data.venues.filter(v =>
              v.venueName === res.data.venue.venueName &&
              v.location === res.data.venue.location
            );
  ...
  const calculatePrice = (slot) => {
    if (!slot || !venue) return 0;
    const start = parseInt(slot.startTime.split(':')[0]);
    const end = parseInt(slot.endTime.split(':')[0]);
    const hours = end - start;
    return hours * venue.price;
  };
  ...
  const handleBookNow = async () => {
    ...
    if (isSlotBooked(selectedDate, selectedSlot.startTime, selectedSlot.endTime)) {
        ...
        <div className="p-6">
          ...
          {availableDates.length === 0 ? (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
              <svg className="w-16 h-16 mx-auto text-yellow-500 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <>
                  ...
                  {availableDates.map((date) => (
                    ...
                    >
                      ...
                      <div className="text-xs mt-1 opacity-75">
                        {slotsByDate[date]?.length || 0} slot{slotsByDate[date]?.length !== 1 ? 's' : ''}
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-6">
                <label className="block text-sm font-semibold text-gray-700 mb-3">
                  Select Time Slot ({currentSlots.length} slot{currentSlots.length !== 1 ? 's' : ''} available)
                </label>
                ...
                {currentSlots.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {currentSlots.map((slot, index) => {
                      const booked = isSlotBooked(selectedDate, slot.startTime, slot.endTime);
                      ...
                      return (
                        ...
                        >
                          <div className={`font-semibold text-sm ${
                            booked ? 'text-red-700' : isSelected ? 'text-green-700' : 'text-gray-800'
                          }`}>
              ...
              {selectedSlot && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-gray-900 font-bold">
                      {parseInt(selectedSlot.endTime.split(':')[0]) - parseInt(selectedSlot.startTime.split(':')[0])} hour(s)
                    </span>
                  </div>
